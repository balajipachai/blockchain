    ...
    describe('function addMoneyToAccount(uint amount) public pullInMoneyLimit returns (bool)', async () => {
      ...
      describe('Psitive scenarios', async () => {
        ...
        it('Should add 5000$ to user1 account', async () => {
          ...
          balance = await bankContractInstance.balanceOf.call(bankAddress)
          console.log('Bank balance: ', balance.toNumber());
          ...
          let events = await bankContractInstance.allEvents(filterOptions)
          events.get((err, logs) => {
            if (!err) {
